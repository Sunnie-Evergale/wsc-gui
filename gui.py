#!/usr/bin/env python3
# gui.py
# WSC Decompiler GUI - Tkinter implementation with drag-and-drop support
...
class WSCDecompilerGUI(TkinterDnD.Tk):
    def __init__(self):
        super().__init__()

        self.title("WSC Decompiler GUI")
    ...
    def setup_widgets(self):
        ...
        # Drag and drop area
        self.drop_area = tk.Label(control_frame, text="Drag & Drop WSC files here\n\n支持拖拽 .WSC 文件",
                                 bg="lightgray", relief="sunken", height=5)
        self.drop_area.grid(row=1, column=0, columnspan=3, sticky="ew", pady=(0, 10))
    ...
    def handle_drop(self, event):
        ...
        for file_path in files:
            ...
            if os.path.isfile(file_path) and file_path.lower().endswith('.wsc'):
                self.add_file(file_path)
            else:
                self.log(f"Invalid file: {os.path.basename(file_path)}", error=True)
    ...
    def show_about(self):
        """Show about dialog."""
        about_text = """WSC Decompiler GUI v1.0
